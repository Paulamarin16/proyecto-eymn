...
$ID_S = isset($_GET['ID_S']) ? $_GET['ID_S'] : '';
...
if ($ID_S == '' || $token == '') {
  ...
  exit;
    ...
    if ($token == $token_tmp) {
      ...
      $sql = $con-> prepare("SELECT count(ID_S) FROM servicios WHERE ID_S=? AND activo=1");
      $sql ->execute([$ID_S]);
      if ($sql->fetchColumn() > 0) {

        $sql = $con->prepare("SELECT Nombre, Tipo, Precio, Descuento, Imagen FROM servicios WHERE ID_S=? AND activo=1
        LIMIT 1");
        $sql->execute([$ID_S]);
        ...
        <div class="col-md-6 order-md-2">
          ...
          <?php if($descuento > 0) { ?>
              ...
              <div class="d-grid gap-3 col-10 mx-auto"> 
                  <button class="btn btn-outline-primary" type="button" onclick="addProducto(<?php echo $ID_S; ?>,
